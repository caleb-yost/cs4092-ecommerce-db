...
| File | Deliverable |
|---|---|
| [requirements.md](requirements.md) / requirements.pdf | Requirements gathering |
| [er_diagram.md](er_diagram.md) / er_diagram.png | ER diagram |
| [schema.md](schema.md) / schema.pdf | Relational schema |
...
To run the standalone queries against the same database:

```bash
sqlite3 ecommerce.db < queries.sql
```

(or open `ecommerce.db` in any SQLite browser / run the statements from `queries.sql`
